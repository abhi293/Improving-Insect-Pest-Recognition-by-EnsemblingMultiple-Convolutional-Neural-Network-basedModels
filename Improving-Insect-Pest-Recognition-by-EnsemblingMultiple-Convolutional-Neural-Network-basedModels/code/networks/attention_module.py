...
class AttentionModule_stage1(nn.Module):
    ...
    def forward(self, x):
        ...
        out_mpool3 = self.mpool3(out_softmax2)
        out_softmax3 = self.softmax3_blocks(out_mpool3)
        out_interp3 = F.interpolate(out_softmax3, size=out_softmax2.shape[2:], mode='bilinear', align_corners=False) + out_softmax2
        out = out_interp3 + out_skip2_connection
        out_softmax4 = self.softmax4_blocks(out)
        # Resize out_softmax1 to match the size of self.interpolation2(out_softmax4)
        out_interp2 = self.interpolation2(out_softmax4) + F.interpolate(out_softmax1, size=self.interpolation2(out_softmax4).shape[2:], mode='bilinear', align_corners=False)
        # Resize out_skip1_connection to match the size of out_interp2
        out = out_interp2 + F.interpolate(out_skip1_connection, size=out_interp2.shape[2:], mode='bilinear', align_corners=False)
        out_softmax5 = self.softmax5_blocks(out)
        # Resize out_trunk to match the size of self.interpolation1(out_softmax5)
        out_interp1 = self.interpolation1(out_softmax5) + F.interpolate(out_trunk, size=self.interpolation1(out_softmax5).shape[2:], mode='bilinear', align_corners=False)
        out_softmax6 = self.softmax6_blocks(out_interp1)
        # Resize out_trunk to match the size of out_softmax6
        out = (1 + out_softmax6) * F.interpolate(out_trunk, size=out_softmax6.shape[2:], mode='bilinear', align_corners=False)
        out_last = self.last_blocks(out)
